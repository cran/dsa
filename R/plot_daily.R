...
if(!("dygraphs" %in% rownames(utils::installed.packages())) | !dy)  {
  ...
  if (trend) {
    dygraphs::dyOptions(dygraphs::dyRangeSelector(dygraphs::dygraph(x$output[,c(1:2,4)])),colors=c("#f08927", "#000000","#0062a1"))} else {
  dygraphs::dyOptions(dygraphs::dyRangeSelector(dygraphs::dygraph(x$output[,c(1:2)])),colors=c("#f08927", "#000000"))
}
